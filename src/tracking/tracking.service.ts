...
const MIN_SAVE_DISTANCE_METERS = 10; // don't save history if < 10m moved
  ...
  async syncVehicles() {
    try {
      ...
      for (const item of data) {
        ...
        /* Skips invalid coords & jitter  */
        /* ------------------------------ */

        if (isValidCoord(lat, lng)) {
          // Check last saved history to avoid duplicate/jitter points
          const lastHistory =
            await this.prisma.vehicleLocationHistory.findFirst({
              where: { vehicleId: updatedVehicle.id },
